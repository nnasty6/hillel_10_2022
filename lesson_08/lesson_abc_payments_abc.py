...
from lesson_abc_providers import PayPalAPI, StripeAPI
...
class PaymentProcessor:
    ...
    def _authorize(self) -> UUID:
        ...
        print(f"User {self.user.name} is "
              f"authorized with {self.payment_system}")
    ...
    def buy(self):
        ...
        print(
            f"Buying {self.product.name} for {self.product.price} "
            f"with {self.payment_system}. User : {self.user.name}"
        )
...
def main():
    ...
    # Make payment
    payment_processor = PaymentProcessor(john, micro, stripe)
...
if __name__ == "__main__":
    ...
    # Payment system - the class that encapsulates
    # the logic to communicate with provider
    # Payment process -  the class that aggregates
    # payment system, user and product
    main()
